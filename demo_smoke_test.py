"""
Demo acceptance script: connect → place one market order → log → close → log.

1. Install MT5 terminal and log in to the demo account (or use env credentials).
2. Edit the TEST_* block below (symbol, volume, pips or fixed prices).
3. Set MT5_LOGIN, MT5_PASSWORD, MT5_SERVER in .env or environment (see README).
4. Run: python demo_smoke_test.py
"""
...
def main() -> int:
    ...
    try:
        ...
        if not tick:
            log_line(f"No tick for {TEST_SYMBOL}. Is the symbol visible in Market Watch?")
            return 1
        ...
        if TEST_USE_FIXED_PRICES:
            ...
            log_line(f"Using fixed SL={sl_f} TP={tp_f} (ref entry ~ {entry_ref})")
        else:
            sl_f, tp_f = _sl_tp_from_pips(
                TEST_SYMBOL, TEST_SIDE, entry_ref, TEST_SL_PIPS, TEST_TP_PIPS
            )
            log_line(
        ...
        positions = find_positions(symbol=TEST_SYMBOL, magic=TEST_MAGIC)
        if not positions:
            positions = find_positions(symbol=TEST_SYMBOL, magic=None)
            log_line(
                f"No position with magic={TEST_MAGIC}; open positions on {TEST_SYMBOL}: {len(positions)}."
            )
            if len(positions) == 0:
                log_line(
